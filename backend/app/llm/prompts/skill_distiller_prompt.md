...
如果原始文档把终止节点写成 type=terminal，请将该节点生成为 type=response，并把 node_id 放入 terminal_node_ids；不要输出 type=terminal。
...
interruption_policy 必须是字符串值映射；布尔值和数组应序列化为 JSON 文本或改写成等价的清晰策略说明，不要直接输出布尔值或数组。
每个收集信息节点的 instruction 都要说明：用户一次提供多个信息时，需要同时提取并写入对应 slot，不要重复追问已提供的信息；当前节点已满足时直接进入下一缺失信息、当前节点允许的执行动作、知识检索或最终回复。
技能必须形成闭环：不得把“请稍候”“正在处理”“稍后反馈”作为最终可见回复；应明确说明当前流程实际完成的结果，无法完成时如实说明能力边界和可用下一步。
只有原始流程确实要求外部事实、外部系统写入、状态变更或业务处理时，才应设计为调用 available_tools 中合适工具，或在原文允许时明确转人工。原文明确禁止工具或人工转接时，不得添加任何通用工具调用或转人工措辞。
如果流程会产生外部副作用、改变用户资产/权益/状态、提交不可自动撤销的处理，或原始文档明确要求确认，必须在进入相应处理或最终回复前增加一个确认节点，确认关键对象、范围和操作内容；用户明确确认后才能继续。
...
终止节点必须允许 answer_user，并要求给用户明确结果；无法完成时应说明能力边界、缺失信息和可用下一步，不得默认转人工或承诺稍后继续。
response_rules 必须包含闭环约束：不得只回复请稍候；必须给出实际完成结果或明确能力边界。仅当流程包含工具调用时，才要求工具成功后必须给出最终业务结果。
response_rules 必须包含自适应推进约束：步骤是目标不是脚本；已满足的信息不得重复追问；模型应推进到下一缺失信息、当前节点允许的执行动作或最终回复。
